...
public final class Api {
     ...
     * MBHFile extension
     **/
    public static final String MBFireWallExt = "MBFireWall";
    /**
     ...
     */
    private static final String SCRIPT_FILE = MBFireWallExt + ".sh";

    // Preferences
    public static final String PREFS_NAME = MBFireWallExt + "Prefs";
    ...
    // Cached applications
    public static MBFireWallApp applications[] = null;
    ...
    private static String scriptHeader(Context ctx) {
        final String busyBox = "/busybox_mbh";
        final String dir = ctx.getDir("bin", 0).getAbsolutePath();
        final String myiptables = dir + "/iptables_mbh";
        return "" +
                "IPTABLES=iptables\n" +
                "BUSYBOX=busybox\n" +
                "GREP=grep\n" +
                "ECHO=echo\n" +
                "# Try to find busybox\n" +
                "if " + dir + busyBox + " --help >/dev/null 2>/dev/null ; then\n" +
                "	BUSYBOX=" + dir + busyBox + "\n" +
                "	GREP=\"$BUSYBOX grep\"\n" +
                "	ECHO=\"$BUSYBOX echo\"\n" +
                ...
                "	if ! $ECHO 1 | $GREP -q 1 >/dev/null 2>/dev/null ; then\n" +
                "		$ECHO The grep command is required. " + MBFireWallExt + " will not work.\n" +
                "		exit 1\n" +
                "	fi\n" +
                "fi\n" +
     ...
     */
    private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors, boolean sameRulesWifi3G) {
        if (ctx == null) {
            return false;
        }
...
//        final boolean logenabled = ctx.getSharedPreferences(PREFS_NAME, 0).getBoolean(PREF_LOGENABLED, false);
        ...
        try {
            ...
            script.append("" +
                    ...
                    "# Create the " + MBFireWallExt + " chains if necessary\n" +
                    "$IPTABLES -L " + MBFireWallExt + " >/dev/null 2>/dev/null || $IPTABLES --new " + MBFireWallExt + " || exit 2\n" +
                    "$IPTABLES -L " + MBFireWallExt + "-3g >/dev/null 2>/dev/null || $IPTABLES --new " + MBFireWallExt + "-3g || exit 3\n" +
                    "$IPTABLES -L " + MBFireWallExt + "-wifi >/dev/null 2>/dev/null || $IPTABLES --new " + MBFireWallExt + "-wifi || exit 4\n" +
                    "$IPTABLES -L " + MBFireWallExt + "-reject >/dev/null 2>/dev/null || $IPTABLES --new " + MBFireWallExt + "-reject || exit 5\n" +
                    "# Add " + MBFireWallExt + " chain to OUTPUT chain if necessary\n" +
                    "$IPTABLES -L OUTPUT | $GREP -q " + MBFireWallExt + " || $IPTABLES -A OUTPUT -j " + MBFireWallExt + " || exit 6\n" +
                    "# Flush existing rules\n" +
                    "$IPTABLES -F " + MBFireWallExt + " || exit 7\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-3g || exit 8\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-wifi || exit 9\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-reject || exit 10\n" +
                    "");
            // Check if logging is enabled
            if (logenabled) {
                script.append("" +
                        "# Create the log and reject rules (ignore errors on the LOG target just in case it is not available)\n" +
                        "$IPTABLES -A " + MBFireWallExt + "-reject -j LOG --log-prefix \"[" + MBFireWallExt + "] \" --log-uid\n" +
                        "$IPTABLES -A " + MBFireWallExt + "-reject -j REJECT || exit 11\n" +
                        "");
            } else {
                script.append("" +
            ...
            if (whitelist && logenabled) {
                ...
                script.append("$IPTABLES -A " + MBFireWallExt + " -p udp --dport 53 -j RETURN\n");
            }
            script.append("# Main rules (per interface)\n");
            for (final String itf : ITFS_3G) {
                script.append("$IPTABLES -A " + MBFireWallExt + " -o ").append(itf).append(" -j " + MBFireWallExt + "-3g || exit\n");
            }
            for (final String itf : ITFS_WIFI) {
                script.append("$IPTABLES -A " + MBFireWallExt + " -o ").append(itf).append(" -j " + MBFireWallExt + "-wifi || exit\n");
            }

            script.append("# Filtering rules\n");
            final String targetRule = (whitelist ? "RETURN" : MBFireWallExt + "-reject");
            ...
            if (whitelist && !any_wifi) {
                ...
                if (uid != -1) {
                    ...
                    script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -m owner --uid-owner ").append(uid).append(" -j RETURN || exit\n");
                }
            ...
            if (any_3g) {
                if (blacklist) {
                    ...
                    script.append("$IPTABLES -A " + MBFireWallExt + "-3g -j ").append(targetRule).append(" || exit\n");
                }
            } else {
                /* release/block individual applications on this interface */
                for (final Integer uid : uids3g) {
                    if (uid >= 0)
                        script.append("$IPTABLES -A " + MBFireWallExt + "-3g -m owner --uid-owner ").append(uid).append(" -j ").append(targetRule).append(" || exit\n");
                }
            }
            if (any_wifi) {
                if (blacklist) {
                    /* block any application on this interface */
                    script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -j ").append(targetRule).append(" || exit\n");
                }
            } else {
				/* release/block individual applications on this interface */
                for (final Integer uid : uidsWifi) {
                    if (uid >= 0)
                        script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -m owner --uid-owner ").append(uid).append(" -j ").append(targetRule).append(" || exit\n");
                }
            }
            if (whitelist) {
                if (!any_3g) {
                    if (uids3g.indexOf(SPECIAL_UID_KERNEL) >= 0) {
                        ...
                        script.append("$IPTABLES -A " + MBFireWallExt + "-3g -m owner --uid-owner 0:999999999 -j " + MBFireWallExt + "-reject || exit\n");
                    } else {
                        script.append("$IPTABLES -A " + MBFireWallExt + "-3g -j " + MBFireWallExt + "-reject || exit\n");
                    }
                }
                if (!any_wifi) {
                    if (uidsWifi.indexOf(SPECIAL_UID_KERNEL) >= 0) {
                        script.append("# hack to allow kernel packets on white-list\n");
                        script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -m owner --uid-owner 0:999999999 -j " + MBFireWallExt + "-reject || exit\n");
                    } else {
                        script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -j " + MBFireWallExt + "-reject || exit\n");
                    }
                }
            } else {
                if (uids3g.indexOf(SPECIAL_UID_KERNEL) >= 0) {
                    script.append("# hack to BLOCK kernel packets on black-list\n");
                    script.append("$IPTABLES -A " + MBFireWallExt + "-3g -m owner --uid-owner 0:999999999 -j RETURN || exit\n");
                    script.append("$IPTABLES -A " + MBFireWallExt + "-3g -j " + MBFireWallExt + "-reject || exit\n");
                }
                if (uidsWifi.indexOf(SPECIAL_UID_KERNEL) >= 0) {
                    script.append("# hack to BLOCK kernel packets on black-list\n");
                    script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -m owner --uid-owner 0:999999999 -j RETURN || exit\n");
                    script.append("$IPTABLES -A " + MBFireWallExt + "-wifi -j " + MBFireWallExt + "-reject || exit\n");
                }
            }
            ...
            if (showErrors && code != 0) {
                ...
                Log.e("" + MBFireWallExt + "", msg);
                // Remove unnecessary help message from output
                if (msg.contains("\nTry `iptables -h' or 'iptables --help' for more information.")) {
                    msg = msg.replace("\nTry `iptables -h' or 'iptables --help' for more information.", "");
                }
                logError("Error applying iptables rules. Exit code: " + code + "\n\n" + msg.trim());
            } else {
                return true;
            }
        } catch (Exception e) {
            if (showErrors) logError("error refreshing iptables: " + e);
        }
        return false;
    }
    ...
    public static boolean applySavedIptablesRules(Context ctx, boolean showErrors) {
        ...
        return applyIptablesRulesImpl(ctx, uids_wifi, uids_3g, showErrors, false);
    }
    ...
    public static void saveRules(Context ctx) {
        final SharedPreferences prefs = ctx.getSharedPreferences(PREFS_NAME, 0);
        final MBFireWallApp[] apps = getApps(ctx);
    ...
    public static boolean purgeIptables(Context ctx, boolean showErrors) {
        ...
        try {
            ...
            script.append(scriptHeader(ctx));
            script.append("" +
                    "$IPTABLES -F " + MBFireWallExt + "\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-reject\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-3g\n" +
                    "$IPTABLES -F " + MBFireWallExt + "-wifi\n" +
                    "");
            if (customScript.length() > 0) {
                script.append("\n# BEGIN OF CUSTOM SCRIPT (user-defined)\n");
                script.append(customScript);
            ...
            if (code == -1) {
                if (showErrors)
                    logError("Error purging iptables. exit code: " + code + "\n" + res);
                return false;
            }
            return true;
        } catch (Exception e) {
            if (showErrors) logError("Error purging iptables: " + e);
            return false;
        }
    }
    ...
    /**
     * Return iptables rules output as String
     *
     * @param ctx application context
     */
    public static String getIptablesRules(Context ctx) {
    ...
    public static boolean clearLog(Context ctx) {
        try {
            ...
            logDebug("Log has been cleared with code: " + code);
            return true;
        } catch (Exception e) {
            logError("error: " + e);
        }
    ...
    public static void showLog(Context ctx) {
        try {
            ...
            int start, end;
            Integer appid;
            final HashMap<Integer, LogInfo> map = new HashMap<>();
            LogInfo loginfo = null;
            while ((line = r.readLine()) != null) {
                if (!line.contains("[" + MBFireWallExt + "]")) continue;
                appid = unknownUID;
    ...
    /**
     * To return list of all installed apps on the current system
     *
     * @param ctx application context (mandatory)
     * @return a list of applications
     */
    public static MBFireWallApp[] getApps(Context ctx) {
        ...
        try {
            ...
            final HashMap<Integer, MBFireWallApp> map = new HashMap<>();
            final Editor edit = prefs.edit();
            boolean changed = false;
            String name = null;
            String cachekey = null;
            MBFireWallApp app = null;
            ...
            final MBFireWallApp special[] = {
                    new MBFireWallApp(SPECIAL_UID_ANY, "(Any application) - Same as selecting all applications", false, false),
                    new MBFireWallApp(SPECIAL_UID_KERNEL, "(Kernel) - Linux kernel", false, false),
                    new MBFireWallApp(android.os.Process.getUidForName("root"), "(root) - Applications running as root", false, false),
                    new MBFireWallApp(android.os.Process.getUidForName("media"), "Media server", false, false),
                    new MBFireWallApp(android.os.Process.getUidForName("vpn"), "VPN networking", false, false),
                    new MBFireWallApp(android.os.Process.getUidForName("shell"), "Linux shell", false, false),
                    new MBFireWallApp(android.os.Process.getUidForName("gps"), "GPS", true, true),
            };
            ...
            applications = map.values().toArray(new MBFireWallApp[map.size()]);
    ...
    public static boolean hasRootAccess(final Context ctx, boolean showErrors) {
        ...
        if (showErrors) {
            logError("Could not acquire root access.\n" +
                    "You need a rooted phone to run " + MBFireWallExt + ".\n\n" +
                    "If this phone is already rooted, please make sure " + MBFireWallExt + " has enough permissions to execute the \"su\" command.\n" +
...
//            logDebug("Processor is " + abi);
            File file = new File(ctx.getDir("bin", 0), "iptables_mbh");
            if (!file.exists()) {
                copyRawFile(ctx, R.raw.iptables_x86, file, "755");
                changed = true;
            }
            // Check busybox
            file = new File(ctx.getDir("bin", 0), "busybox_mbh");
            if (!file.exists()) {
                copyRawFile(ctx, R.raw.busybox_x86, file, "755");
                changed = true;
            }
            if (changed) {
//                Toast.makeText(ctx, "Binaries Installed", Toast.LENGTH_LONG).show();
                logDebug("MBFirewall Binaries have been Installed");
            }
        } catch (Exception e) {
            if (showErrors) logError("Error installing binary files: " + e);
            return false;
        }
        return true;
    }
...
//    /**
//     * Check if the firewall is set for the first time ever
//     *
//     * @param ctx mandatory context
//     * @return boolean true if first time, false otherwise
//     */
//    public static boolean isFirstTime(Context ctx) {
//        if (ctx == null) return false;
//        return ctx.getSharedPreferences(PREFS_NAME, 0).getBoolean(PREF_FIRST_TIME, true);
//    }

//    /**
//     * Set or change the first time variable in sharedPreferences
//     *
//     * @param ctx mandatory context
//     * @return boolean true if first time, false otherwise
//     */
...
//        edit.putBoolean(PREF_FIRST_TIME, firstTime);
//        edit.apply();
//    }
    ...
    private static void logDebug(String message) {
        Log.d(MBFireWallExt, message);
    }

    private static void logError(String message) {
        Log.e(MBFireWallExt, message);
    }
    ...
    public static void setEnabled(Context ctx, boolean enabled) {
        ...
        if (!edit.commit()) {
            logError("Error writing to preferences");
            return;
        }
		/* notify */
        if (enabled) {
            if (Api.applySavedIptablesRules(ctx, false)) {
            } else {
                return;
            }
        } else {
            if (Api.purgeIptables(ctx, false)) {
            } else {
                return;
            }
        }
    ...
    public static void applicationRemoved(Context ctx, int uid) {
        ...
        if (savedUids_wifi.length() > 0) {
            ...
            while (tok.hasMoreTokens()) {
                ...
                if (uid_str.equals(token)) {
                    Log.d(MBFireWallExt, "Removing UID " + token + " from the wi-fi list (package removed)!");
                    changed = true;
                } else {
        ...
        if (savedUids_3g.length() > 0) {
            ...
            while (tok.hasMoreTokens()) {
                final String token = tok.nextToken();
                if (uid_str.equals(token)) {
                    Log.d(MBFireWallExt, "Removing UID " + token + " from the 3G list (package removed)!");
                    changed = true;
                } else {
                    if (newuids.length() > 0) newuids.append('|');
                    newuids.append(token);
